class OpenaiService
  ...
  def stream_chat(messages, max_tokens: 4000, &block)
    url = "https://api.openai.com/v1/chat/completions"
  ...
  def resume_chat(query, conversation = nil)
    ...
    # If we have a conversation, use its messages; otherwise, just use the system message and query
    if conversation
      messages = conversation.messages_for_api
      # Add the user query if it's not already the last message
  ...
  # New method for streaming resume chat responses with conversation history
  def stream_resume_chat(query, conversation = nil, &block)
    ...
    return stream_chat(messages, &block)
  end
